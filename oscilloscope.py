...
class Oscilloscope(): 
    ...
    """ Defines an oscilloscope class and common methods"""

    def __init__(self, time_div: int) -> None: 

        """ Establishes connection to the instruments through the PyVisa Driver 

        Args: 
            time_div (int): argument to set the time division for the oscilloscope
    ...
    def screenshot(self, folder_path: str, args: str) -> Image.Image:

        """ Defines functionality to take a screenshot of the oscilloscope 

        Args: 
            folder_path (str): path of the folder where the screenshot will be stored   

        Outputs: .PNG of the oscilloscope screen 
        """
        self.fname_stem = args
        self.date_string = datetime.strftime(datetime.now(), "%Y%m%d_%H%M%S_")
        self.fname = self.date_string + self.fname_stem + ".png" 
    ...
    def save_csv(self, volts_div, time_div, channel_list, args: str):
        ...
        while self.trigger_ON:
            ...
            if "STOP" in flag: 
                ...
                self.screenshot(folder_path, args)
